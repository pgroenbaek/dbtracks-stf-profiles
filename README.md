...
You can also make all Dynatrax track shapes use a profile by default by adding `Dynatrax-*` to the `IncludeShapes` parameter within one of the profiles. For example from `IncludedShapes ( "DB2f_*" )` to `IncludedShapes ( "DB2f_*, Dynatrax-*" )`. If you want some of the Dynatrax shapes to use a different profile than the default one, you can still rename those Dynatrax shapes with the DBTracks prefixes like discussed above.
