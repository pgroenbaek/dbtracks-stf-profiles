...
For example from `IncludedShapes ( "DB2f_*" )` to `IncludedShapes ( "DB2f_*, Dynatrax-*" )`. If you want some of the Dynatrax shapes to use a different profile than the default one, you can still rename those Dynatrax shapes with the DBTracks prefixes like discussed above.
